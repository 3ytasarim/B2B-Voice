...
// B2BVoice brand palette: white/near-white chassis, primary-blue accents
// (matches the footer's --primary blue and the white logo/text).
...
export interface FooterRobotProps {
  className?: string;
  color?: string;
  pantallaColor?: string;
  pantallaBrillo?: number;
...
/**
 * Just the interactive robot (mouse-follow, blinking, click-to-heart-eyes,
 * glowing screen) with no navbar / hero text — sized for a footer slot.
 */
export function FooterRobot({
  className = "",
  color = "#f4f6fb",
  pantallaColor = "#ffffff",
  pantallaBrillo = 1.4,
  blinkCycle = 3.0,
  metalness = 0.05,
  scale = 1,
  ...
  return (
    <div className={`relative pointer-events-auto ${className}`}>
      <Canvas shadows camera={{ position: [0, 0.2, 6], fov: 40 }}>
        ...
        <directionalLight position={[-5, 2, -5]} intensity={0.25} color="#cbd9ee" />
